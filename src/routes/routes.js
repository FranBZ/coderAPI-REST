...
router.get('/:id', (req, res) => {  // GET '/api/productos/:id' -> devuelve un producto según su id.
    const { id } = req.params
    const prod = productos.filter(prod => prod.id == id)
    if ( prod.length == 0 ) res.status(400).json({ error : 'producto no encontrado' })
    res.send(prod)
})
...
router.post('/', (req, res) => {  // POST '/api/productos' -> recibe y agrega un producto, y lo devuelve con su id asignado.
    ...
    if ( !nombre || !precio || !urlImage ) res.status(400).json({ error : 'por favor ingrese todos los datos' })
...
router.put('/:id', (req, res) => {  // PUT '/api/productos/:id' -> recibe y actualiza un producto según su id.
    const { id } = req.params
    const { nombre, precio, urlImage } = req.body
    if ( !nombre || !precio || !urlImage ) res.status(400).json({ error : 'producto no encontrado' })
    let contador = 0
...
router.delete('/:id', (req, res) => {  // DELETE '/api/productos/:id' -> elimina un producto según su id.
    const { id } = req.params
    ...
    if ( productos.length == largoActual ) res.status(400).json({ error : 'producto no encontrado' })
    res.send(productos)
})
